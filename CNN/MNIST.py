...
# 28*28的图片对象
# 根据需求是否转化为784的向量
# 每个对象的标签为0-9的数字，one-hot编码为10维向量
import numpy as np

# 数据加载器基类，派生出图片加载器和标签加载器
class Loader(object):
    # 初始化加载器 path:文件路径 count：文件中样本数目
    ...
    def get_file_content(self):
        ...
        f.close()
        return content
    # 将unsigned byte字符转为整数，python3中bytes的每个分量读取就变成int
    # def to_int(self,byte):
    #     return struct.unpack('B',byte)[0]

# 图像数据加载器
class ImageLoader(Loader):
    # 内部函数，从文件字节数组读取第index个图像数据。文件中包含所有样本图片的数据
...
class LabelLoader(Loader):
    ...
    def load(self):
        ...
        for index in range(self.count): # 遍历每一个样本
            ...
            onelablevec=self.norm(onelable)# one-hot编码
...
# 获取训练数据集。onerow表示是否将每张图片转换为行向量
def get_training_data_set(num,onerow=False):
    imager_loader=ImageLoader('train-images.idx3-ubyte',num)# 参数为文件路径和加载的样本数量
    label_loader=LabelLoader('train-labels.idx1-ubyte',num)# 参数为文件路径和加载的样本数量
    return imager_loader.load(onerow),label_loader.load()

# 获得测试数据集，onerow表示是否将每张图片转化为行向量
def get_test_data_set(num,onerow=False):
    image_loader=ImageLoader('t10k-images.idx3-ubyte',num)
    lable_loader=LabelLoader('t10k-labels.idx1-ubyte',num)
    return image_loader.load(onerow),lable_loader.load()
