...
# 全连接神经网络可以包含多层，但是只有最后一层输出前有激活函数
# 所谓向量化编程，就是使用矩阵运算
import random
import math
import numpy as np
...
class FullConnectedLayer(object):
    # 全连接层构造函数。input_size:本层输入列向量的维度，
    # output_szie:本层输出的列维度
    # activator：激活函数
    def __init__(self,input_size,output_size,activator,learning_rate):
        self.input_size=input_size
        self.output_size=output_size
        self.activator=activator
        # 权重数组W，初始化权重为(rand(output_size,input_size)-0.5)
    ...
    # 前向计算，预测输出.
    # input_array: 输入列向量，维度必须等于input_size
